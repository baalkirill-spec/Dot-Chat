...
fun CreateChatRoute(
    ...
    LaunchedEffect(state.createdChatId) {
        state.createdChatId?.let(onCreated)
    }
    ...
    LazyColumn(
            ...
            StreamSectionCard(modifier = Modifier.fillMaxWidth()) {
                androidx.compose.foundation.layout.Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                    StreamTextField(
                        ...
                        label = stringResource(R.string.create_chat_name),
                    )
                    StreamTextField(
                        value = state.description,
                        onValueChange = viewModel::onDescriptionChanged,
                        label = stringResource(R.string.create_chat_description),
                        singleLine = false,
                    )
        ...
        item {
            StreamPrimaryButton(
                text = stringResource(R.string.create_chat_continue),
                modifier = Modifier.fillMaxWidth(),
